...
export function RetroGrid({
  ...
  cellSize = 60,
  opacity = 0.5,
  lightLineColor = "#a1a1a1", // Adjust to slightly darker grey for better visibility
  darkLineColor = "#a1a1a1",
  style = {},
  ...props
}) {
  ...
  useEffect(() => {
    ...
    setIsDark(isDarkMode(colorScheme));
  ...
  const activeFallbackGridStyles = {
    ...createFallbackGridStyle(
      normalizedCellSize,
      isDark ? darkLineColor : lightLineColor
    ),
    position: "absolute",
  ...
  return (
    <div ref={containerRef} className={className} style={gridStyles} {...props}>
      <style>{FALLBACK_STYLES}</style>
      
      {!isWebGlReady ? (
        <div style={fallbackProjectionStyles}>
          <div style={fallbackRotationStyles}>
            <div data-retro-grid-scroll="true" style={activeFallbackGridStyles} />
          </div>
        </div>
      ) : null}
      ...
      {/* Gradient overlay to fade it out at the top/bottom (like the original Tailwind bg-linear-to-t) */}
      <div 
        style={{
          position: "absolute",
          inset: 0,
          background: "linear-gradient(to top, var(--bg-primary), transparent 90%)"
        }} 
      />
    </div>
  );
